...
freebsd() {
  GUEST=${1:-freebsd-Release-zfs} ; variant=freebsd
  ...
  #set DEVX=[da0|vtbd0] ; gpart show -l
...
devuan() {
  GUEST=${1:-devuan-Stable-lvm} ; variant=debian
...
voidlinux() {
  GUEST=${1:-voidlinux-Rolling-lvm} ; variant=voidlinux
...
artix() {
  GUEST=${1:-artix-Rolling-lvm} ; variant=archlinux
...
alpine() {
  GUEST=${1:-alpine-Stable-lvm} ; variant=alpine
...
debian() {
  GUEST=${1:-debian-Stable-lvm} ; variant=debian
...
archlinux() {
  GUEST=${1:-archlinux-Rolling-lvm} ; variant=archlinux
...
opensuse() {
  GUEST=${1:-opensuse-Stable-lvm} ; variant=suse
...
centos() {
  GUEST=${1:-centos-Release-lvm} ; variant=redhat
...
mageia() {
  GUEST=${1:-mageia-Release-lvm} ; variant=mageia
...
pclinuxos() {
  GUEST=${1:-pclinuxos-Rolling-lvm} ; variant=pclinuxos
...
#  export DEVX=[sv]da ; [[sgdisk -p | sfdisk -l] /dev/$DEVX | parted /dev/$DEVX -s unit GiB print]
...
netbsd() {
  GUEST=${1:-netbsd-Release-std} ; variant=netbsd
...
openbsd() {
  GUEST=${1:-openbsd-Release-std} ; variant=openbsd
