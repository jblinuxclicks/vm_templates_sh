...
#urpmi --no-recommends --auto --urpmi-root /mnt mageia-release urpmi dnf locales-en
dnf --setopt=install_weak_deps=False -y --installroot=/mnt --releasever=${VERSION_ID} install mageia-repos
dnf -y --installroot=/mnt --releasever=${VERSION_ID} check-update
dnf --setopt=install_weak_deps=False -y --installroot=/mnt --releasever=${VERSION_ID} install mageia-release urpmi dnf dnf-plugins-core locales-en lvm2
...
dnf -y config-manager --set-enabled \${ID}-${BASEARCH} updates-${BASEARCH}
...
pkgs_nms="basesystem kernel-desktop-latest microcode_ctl locales-en sudo dhcp-client man-pages dosfstools openssh-server nano mandi-ifw shorewall shorewall-ipv6 urpmi dnf dnf-plugins-core harddrake-ui grub grub2-efi efibootmgr lvm2" # task-lxde"
#urpmi.update -a
